...
export async function writeArticle(item: KeywordItem, aff: Affiliates): Promise<{ body: string; description: string | null; title: string | null; tools: Tool[] }> {
  ...
  if (item.template.startsWith("news:")) {
    ...
    // ニュースの会話にスクール記事の定型（返金保証・サポート条件）が紛れ込む（9/23 実測）ので落とす
    const okLine = (l: string) => numbersIn(l).every((x) => matNums.has(x)) && !/という声|との口コミ|口コミ(が|も)多い|口コミでは|と評判|受講生の声|最大\s*\d+\s*万円|最大\s*\d+\s*[%％]|返金保証|サポート条件/.test(l);
    const kg = await chat(
